<template>
    <div class="main">
        <h3 style="margin-top: 0;">WebSocket Test</h3>
...
export default {
    ...
    data() {
        return {
            ...
            serverUrl: `${process.env.baseWebsocketUrl}/websocket`,
            websocket: null,
            ...
            switch(message.type) {
                case 'new_user_login':
                    if(message.data.name !== this.form.name) {
                        this.online.push(message.data.name);
                    }
                    break;
                case 'user_logout':
                    let index = this.online.indexOf(message.data.name);
                    if(index !== -1) {
